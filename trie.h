...
public:
    ArrayUnit() : index_(0), label_(0), eow_(false), parent_(0) {}

    bool Eow() const { return eow_; }
    int32_t Value() const { return (HasValue()) ? value_ : 0; }
    uint8_t Label() const { return label_; }
    uint32_t Index() const { return (!HasValue()) ? index_ : 0; }
    uint32_t Parent() const { return parent_; }
    bool HasValue() const { return label_ == '\0' && eow_; }
    bool IsEmpty() const { return index_ == 0 && label_ == 0 && !eow_ && parent_ == 0; }

    void SetEow(bool e) { eow_ = e; }
    void SetValue(int32_t v) { value_ = v; label_ = '\0'; eow_ = true; }
    void SetLabel(uint8_t label) { label_ = label; }
    void SetIndex(uint32_t offset) { index_ = offset; }
    ...
    std::vector<SearchResult> PrefixSearch(const std::string& str,
        ...
        if (array_[0].Eow()) {
            ...
            if (value_pos < size_ && array_[value_pos].HasValue()) {
                results.emplace_back(static_cast<T>(array_[value_pos].Value()), 0);
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            if (next_unit.Eow()) {
                ...
                if (value_pos < size_ && array_[value_pos].HasValue()) {
                    results.emplace_back(static_cast<T>(array_[value_pos].Value()), i + 1);
